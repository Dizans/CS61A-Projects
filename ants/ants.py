...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            elif type(self.ant) == QueenAnt:
                if self.ant.impostor_state == False:
                    return
                else:
                    self.ant = None
            else:
    ...
    def nearest_bee(self, hive):
        ...
        if self.place.bees:
            ...
            while current_place != hive:
                if (len(current_place.bees) != 0) and rng <= mx and rng >= mn:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        current_place = self.place
        if not self.digesting and (len(current_place.bees) != 0):
            self.eat_bee(random_or_none(current_place.bees))
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    num_queen = 0
    doubled_list = []
    impostor_state = False

    def __init__(self):
        ThrowerAnt.__init__(self, 1)
        QueenAnt.num_queen += 1
        self.num_queen = QueenAnt.num_queen
        if QueenAnt.num_queen > 1:
            self.impostor_state = True
    ...
    def action(self, colony):
        ...
        if self.num_queen > 1:
            ...
            while current_place.entrance != None:
                if current_place.ant != None and current_place.ant not in self.doubled_list and current_place.ant.name != 'Queen':
                    if current_place.ant.name == 'Bodyguard' and current_place.ant.ant.name != 'Queen':
                        current_place.ant.ant.damage = current_place.ant.ant.damage * 2
                    current_place.ant.damage = current_place.ant.damage * 2
                    self.doubled_list.append(current_place.ant)
                current_place = current_place.entrance
