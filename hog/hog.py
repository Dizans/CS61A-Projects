...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        score = 1 + abs(opponent_score%10 - opponent_score//10)
    else:
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    n = 10
    def value(n):
        return make_averaged(roll_dice)(n, dice)
    highest = value(n)
    rolls = n
    while n > 0:
        if value(n) > highest:
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    NUM_ROLLS otherwise.
    """
    if take_turn(0, opponent_score) + score != opponent_score*2:
        if take_turn(0, opponent_score) + score == opponent_score/2:
            num_rolls = 0
        else:
...
def final_strategy(score, opponent_score):
    """We check for hog wild first to cause other player to get the four sided dice
    Then we check for free bacon if close enough to 100+ to win
    Within free bacon we check for exceptions such as Hog Wilds and Swine Swaps
    that would be disadvantageous and change to margin to account for it.
    """
    if (score + opponent_score) % 7 == 0:
        num_rolls = 4
        margin = 4
    ...
    # If you're close enough to get it from bacon, get it.
    if bacon + score >= 100:
        return 0

    if score + 1 == opponent_score/2:
        if 2*(opponent_score - (score + 1)) > margin:
            num_rolls = 10
            margin = 2*(opponent_score - (score + 1))
    
    if score + bacon == opponent_score/2:
        if 2*(opponent_score - (score + bacon)) > margin:
            num_rolls = 0
            margin = 2*(opponent_score - (score + bacon))

    if (score + opponent_score + bacon) % 7 == 0:
        if bacon + (8 - 4) > margin:
            num_rolls = 0
