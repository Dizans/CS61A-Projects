...
(define (list-partitions total max-pieces max-value)
  ...
  (cond
    ...
    ((or (= max-value 0) (= max-pieces 0)) nil) 
    (else
      (append (helper (- total max-value) (- max-pieces 1) max-value
        (append partitions (cons max-value nil)))
        (helper total max-pieces (- max-value 1) partitions)))))
