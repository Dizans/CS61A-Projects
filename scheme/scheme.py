...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        """
        frame = Frame(self)
        if len(formals) != len(vals): return SchemeError
        i = 0
        while i != len(vals):
...
def check_formals(formals):
    ...
    unique_symbols = []
    for el in formals:
        if el in unique_symbols or not scheme_symbolp(el): raise SchemeError
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            if isinstance(procedure, PrimitiveProcedure):
                ...
                new_frame = procedure.env.make_call_frame(procedure.formals, args)
                env, expr = new_frame, procedure.body
            elif isinstance(procedure, MuProcedure):
                new_frame = env.make_call_frame(procedure.formals, args)
                env, expr = new_frame, procedure.body
